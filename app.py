...
import ast#for evaluating string as python expressions "1,2"int to list
from textblob import TextBlob  #For sentiment analysis on review
from fuzzywuzzy import process# for misspelling and typos (restaurant name)
import numpy as np
...
def calculate_sentiment(review_list):
    ...
    """
    if pd.isnull(review_list) or not review_list or review_list == '[]':#Check if reviews are missing 
...
def convert_cuisine_string_to_list(cuisine_str):
    ...
    try:
        ...
        return [c.strip() for c in cuisines]# remove extra spaces
    except:
        return []
